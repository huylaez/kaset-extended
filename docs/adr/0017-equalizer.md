...
- **Track-transition pause grace period.** A playback pause retains a running engine for up to 30 seconds. YouTube Music commonly reports a brief paused state between tracks; retaining the process tap avoids an audible interruption from tearing down and recreating the tap. Resuming within the grace period cancels the pending stop. A sustained pause, disabling the EQ, or changing the default output device stops or rebinds the engine immediately as appropriate.
